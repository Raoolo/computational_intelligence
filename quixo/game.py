...
class Game(object):
    ...
    def move(self, from_pos: tuple[int, int], slide: Move, player_id: int) -> bool:
        # public method to execute move on the board
        return self.__move(from_pos, slide, player_id)

    def valid(self, from_pos: tuple[int, int], slide: Move, player_id: int) -> bool:
        # checks if move is valid without executing it
        if player_id > 2:
            return False
        # save previous state to revert changes
        prev_value = deepcopy(self._board)
        # verify with method take (only accept if on borders)
        acceptable = self.__take((from_pos[1], from_pos[0]), player_id)
        if acceptable:  # check if you can actually slide it from the position to the destination
            acceptable = self.__slide((from_pos[1], from_pos[0]), slide)

        self._board = prev_value    # revert state
    ...
    def __take(self, from_pos: tuple[int, int], player_id: int) -> bool:
        '''Take piece, acceptable only if in border'''
        acceptable: bool = (
